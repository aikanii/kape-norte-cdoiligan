...
export const Route = createFileRoute("/guide")({
  loader: ({ context }) => preloadPublicQuery(context.queryClient, guideQuery),
  head: () => ({
    meta: [
...
const CITIES = ["Iligan City", "Cagayan de Oro City"] as const;
...
const drinks: Record<City, Array<{ name: string; note: string }>> = {
  "Iligan City": [
    {
      name: "Spanish latte",
      note: "Sweet, creamy and a reliable first order at modern neighborhood cafes.",
    },
    {
      name: "Iced tablea mocha",
      note: "Look for local cacao paired with espresso for a richer chocolate finish.",
    },
    {
      name: "Pour-over",
      note: "Best for tasting the brighter notes of Philippine single-origin beans.",
    },
  ],
  "Cagayan de Oro City": [
    {
      name: "Cold brew",
      note: "A smooth choice for CDO afternoons and cafes with long opening hours.",
    },
    {
      name: "Sea salt latte",
      note: "A sweet-salty specialty drink found across many contemporary coffee bars.",
    },
    {
      name: "Local robusta espresso",
      note: "Ask which Mindanao-grown beans are on bar for a distinctly regional cup.",
    },
  ],
};

function score(shop: GuideShop) {
  const reviewScore = shop.average_rating
    ? shop.average_rating * 12 + Math.min(shop.review_count, 10) * 2
    : 0;
  const detailScore =
    (shop.google_photo_url ? 8 : 0) +
    shop.tags.length +
    Object.values(shop.hours).filter(Boolean).length;
...
function CoffeeGuide() {
  const query = useQuery({ ...guideQuery, enabled: typeof window !== "undefined", retry: 1 });
  if (query.isPending) return <DirectoryLoading />;
  if (query.isError) return <DataLoadError retry={() => query.refetch()} />;
  return <CoffeeGuideContent shops={query.data} />;
}

function CoffeeGuideContent({ shops }: { shops: GuideShop[] }) {
      ...
      <main>
        <header className="border-b border-border bg-card">
          <div className="mx-auto max-w-6xl px-5 py-10 sm:py-14">
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">
              A practical guide to standout coffee stops in Iligan and CDO, ranked from live reviews
              and listing details.
            </p>
            <div
              className="mt-7 inline-flex rounded-lg border border-border bg-background p-1"
              role="group"
            ...
            >
              {CITIES.map((item) => (
                ...
                >
                  {item === "Cagayan de Oro City" ? "CDO" : "Iligan"}
                </Button>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-6xl px-5 py-10">
          <section aria-labelledby="top-picks">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                >
                  Top picks in {city === "Cagayan de Oro City" ? "CDO" : "Iligan"}
                </h2>
              </div>
              <span className="hidden text-sm text-muted-foreground sm:block">
                Updated from community reviews
              </span>
            </div>
                  ...
                  >
                    ...
                    >
                      <div className="relative aspect-[16/10] bg-muted">
                        ...
                        <span className="absolute left-3 top-3 flex size-8 items-center justify-center rounded-full bg-card text-sm font-semibold text-foreground shadow-sm">
                          {index + 1}
                        </span>
                      </div>
                      <div className="p-4">
                        ...
                        <div className="mt-4 flex items-center justify-between gap-3 border-t border-border pt-3 text-xs">
                          <span
                            className={
                              state.open ? "font-medium text-primary" : "text-muted-foreground"
                            }
                          >
                          ...
                          <span className="flex items-center gap-1 text-foreground">
                            ...
                            />
                            {shop.average_rating
                              ? `${shop.average_rating} (${shop.review_count})`
                              : "New"}
                          </span>
                        </div>
                      </div>
          ...
          <section aria-labelledby="drinks" className="mt-14 border-y border-border py-10">
            ...
            <h2 id="drinks" className="mt-2 font-serif text-3xl font-semibold text-foreground">
              Must-try drinks
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              Menus change, so treat these as drinks to look or ask for around{" "}
              {city === "Cagayan de Oro City" ? "CDO" : "Iligan"}.
            </p>
          ...
          <section aria-labelledby="compare" className="mt-14">
            ...
            <h2 id="compare" className="mt-2 font-serif text-3xl font-semibold text-foreground">
            ...
            <div className="mt-6 overflow-hidden rounded-lg border border-border bg-card">
              ...
              {picks.map((shop) => (
                <Link
                  ...
                  to="/shops/$shopSlug"
                  params={{ shopSlug: shop.slug }}
                  className="grid gap-3 border-b border-border p-4 last:border-b-0 hover:bg-muted/60 md:grid-cols-[1.5fr_.5fr_1fr_1fr] md:items-center md:gap-4"
                >
                  <div>
                  ...
                  <div>
                    <span className="mr-2 text-xs text-muted-foreground md:hidden">Price</span>
                    <span className="font-medium text-primary">{peso(shop.price_level)}</span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    <span className="mr-1 text-xs text-muted-foreground md:hidden">Vibe</span>
                    ...
                    {shop.tags.length === 0 && (
                      <span className="text-sm text-muted-foreground">Coffee stop</span>
                    )}
                  </div>
                  <div className="flex items-start gap-2 text-sm text-foreground">
                    <Clock3 className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" />
                    <span>{todayHours(shop)}</span>
                  </div>
                </Link>
              ))}
            </div>
