...
export const Route = createFileRoute("/")({
  loader: ({ context }) => preloadPublicQuery(context.queryClient, shopsQuery),
  head: () => ({
    meta: [
...
function Directory() {
  const query = useQuery({ ...shopsQuery, enabled: typeof window !== "undefined", retry: 1 });
  if (query.isPending) return <DirectoryLoading />;
  if (query.isError) return <DataLoadError retry={() => query.refetch()} />;
  return <DirectoryContent shops={query.data} />;
}

function DirectoryContent({ shops }: { shops: Shop[] }) {
  const now = useNow();
  ...
  const tags = useMemo(() => Array.from(new Set(shops.flatMap((s) => s.tags))).sort(), [shops]);
  ...
  const results = useMemo(() => {
    return shops.filter((s: Shop) => {
      ...
      if (openOnly && !openState(s.hours, now).open) return false;
      if (query.trim()) {
  ...
  }, [shops, city, area, tag, openOnly, query, now]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="relative flex min-h-[88vh] flex-col overflow-hidden">
        <img
          src={heroImage}
          alt="Latte on a warm specialty coffee bar"
          width={1920}
          height={1280}
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col justify-center px-5 pb-16 pt-16 sm:pb-20">
          <p className="mb-5 text-xs font-semibold uppercase tracking-[0.28em] text-primary">
            The specialty coffee directory of the north
          </p>
          <h1 className="max-w-4xl font-serif text-4xl font-normal leading-[1.08] text-foreground sm:text-6xl lg:text-7xl">
            Specialty Coffee Shops in{" "}
            <span className="italic text-primary">Iligan & Cagayan de Oro</span>
          </h1>
          <p className="mt-6 max-w-xl text-base font-light leading-7 text-foreground/75 sm:text-lg">
            Discover independent cafes and neighborhood coffee spots across Iligan City and Cagayan
            de Oro.
          </p>

          <section
            aria-label="Find coffee shops"
            className="glass-panel mt-9 max-w-5xl rounded-[28px] p-3 sm:p-4"
          >
            <div className="grid gap-2 md:grid-cols-[1.5fr_1fr_1fr_1fr]">
              <label className="flex min-w-0 flex-col gap-1.5">
                <span className="ml-1 text-[11px] font-semibold uppercase tracking-wider text-foreground/70">
                  Search
                </span>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Cafe, area, or street"
                    aria-label="Search by cafe name, area, or street"
                    className="glass-field h-12 w-full min-w-0 rounded-2xl pl-11 pr-4 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary"
                  />
                </div>
              </label>
              ...
              <label className="flex min-w-0 flex-col gap-1.5">
                ...
                <select
                  ...
                  className="glass-field h-12 w-full min-w-0 rounded-2xl px-4 text-sm text-foreground outline-none focus:border-primary"
                >
              ...
              <label className="flex min-w-0 flex-col gap-1.5">
                ...
                <select
                  ...
                  aria-label="Filter by area or neighborhood"
                  className="glass-field h-12 w-full min-w-0 rounded-2xl px-4 text-sm text-foreground outline-none focus:border-primary"
                >
              ...
              <label className="flex min-w-0 flex-col gap-1.5">
                ...
                <select
                  ...
                  aria-label="Filter by vibe or atmosphere"
                  className="glass-field h-12 w-full min-w-0 rounded-2xl px-4 text-sm text-foreground outline-none focus:border-primary"
                >
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 px-1">
              ...
              <p className="text-xs text-muted-foreground">
                <span className="font-semibold text-foreground">{shops.length}</span> cafes curated
              </p>
            </div>
          </section>
          <a
            href="#directory"
            className="mt-8 inline-flex w-fit items-center gap-2 text-xs uppercase tracking-[0.2em] text-foreground/55 transition-colors hover:text-primary"
          >
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="min-w-0 flex-1 py-1">
                          ...
                          <p className="mt-2 line-clamp-2 text-sm font-light leading-5 text-foreground/70">
                            {shop.blurb}
                          </p>
                        </div>
